...
colnames(L19_V2) <- make.names(Level19Codes$Name)
...
Land_categorization_total <- L19_V2 %>%
  group_by(HH_ID) %>%
  summarise(
    land_possessed_acres_total = sum(area.of.land..0.00.acre.[Srl.No. %in% c(1,2,3,4,6,7,8,9)], na.rm = TRUE)
  )
...
Land_categorization_total$land_possessed_ha_total <- round(
  Land_categorization_total$land_possessed_acres_total *  0.405, 
  3)
...
Land_categorization_total <- Land_categorization_total %>% 
  mutate(
    size_class_of_land_possessed_ha_total = cut(
      land_possessed_ha_total, 
...
Common_HH_Basic_with_Land_total <- Common_HH_Basic %>% 
  left_join(
    Land_categorization_total %>%
...
# Replace NAs with 0 to be sure
Common_HH_Basic_with_Land_total[is.na(Common_HH_Basic_with_Land_total)] <- 0
